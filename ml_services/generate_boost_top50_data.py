"""
Boost Top 4-53 Users' Transaction Counts by 8x
================================================
Queries ranks #4-53 from the all-time personal leaderboard and generates
synthetic transactions so each user's total becomes current_count * 8.
Reuses all infrastructure from generate_low_activity_data.py.

Usage:
    python generate_boost_top50_data.py           # Dry-run (default)
    python generate_boost_top50_data.py --execute  # Actually insert
"""
...
MULTIPLIER = 8  # Target: current_count * 8
...
def generate_ref_id(txn_date, format_type, used_ref_ids):
    ...
    for _ in range(100):
        ...
        if ref_id not in used_ref_ids:
            ...
            return ref_id
    # Fallback with BOOST prefix
    ref_id = f"BOOST{date_str}{random_digits(12)}"
    used_ref_ids.add(ref_id)
    return ref_id
...
# ── Phase 1: Identify Top 4-53 Users ────────────────────────────────────────

def identify_target_users(cursor):
    """Get ranks #4-53 from the all-time personal leaderboard."""
    cursor.execute("""
...
def learn_global_patterns(cursor):
    """Learn hour/dow/amount/ref_id distributions from top users (50+ txns)."""
    cursor.execute("""
...
def compute_generation_targets(target_users):
    """For each user, compute current_count * 7 new transactions (total = 8x)."""
    plan = []
    for user in target_users:
        existing = user['txn_count']
        target_total = existing * MULTIPLIER
        to_generate = target_total - existing  # = existing * 7
        plan.append((
            user['user_id'],
            user['name'],
            existing,
...
def validate_results(cursor, generation_plan):
    ...
    rows = cursor.fetchall()
    # Build lookup for expected targets
    expected = {g[0]: g[3] for g in generation_plan}  # user_id -> target_total

    print(f"\n  {'User':>8}  {'Total':>6}  {'Target':>6}  {'Sep':>5}  {'Oct':>5}  {'Nov':>5}  {'Dec':>5}  {'Status':>8}")
    print(f"  {'─'*8}  {'─'*6}  {'─'*6}  {'─'*5}  {'─'*5}  {'─'*5}  {'─'*5}  {'─'*8}")

    all_ok = True
    for row in rows:
        uid = row['user_id']
        total = int(row['total'] or 0)
        target = expected.get(uid, 0)
        sep = int(row['sep_ct'] or 0)
        oct_ = int(row['oct_ct'] or 0)
        nov = int(row['nov_ct'] or 0)
        dec = int(row['dec_ct'] or 0)
        # Allow 5% tolerance
        ok = abs(total - target) <= max(target * 0.05, 10)
        status = "OK" if ok else "CHECK"
        if not ok:
            all_ok = False
        print(f"  #{uid:>7}  {total:>6}  {target:>6}  {sep:>5}  {oct_:>5}  {nov:>5}  {dec:>5}  {status:>8}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Boost Top 4-53 users' transaction counts by 8x")
    ...
    print("=" * 60)
    print("Boost Top 4-53 Transaction Generator")
    print(f"Mode: {'DRY RUN' if dry_run else 'EXECUTE'}")
    print(f"Multiplier: {MULTIPLIER}x")
    print("=" * 60)

    # Phase 1
    print("\n[1/6] Connecting and identifying target users (ranks #4-53)...")
    ...
    print(f"\n  {'Rank':>4}  {'User ID':>8}  {'Name':>30}  {'Existing':>8}  "
          f"{'Target':>8}  {'Generate':>8}")
    print(f"  {'─'*4}  {'─'*8}  {'─'*30}  {'─'*8}  {'─'*8}  {'─'*8}")
    for rank, (uid, name, existing, target, to_gen) in enumerate(generation_plan, 4):
    ...
    if not dry_run:
        ...
        all_ok = validate_results(cursor, generation_plan)
    else:
    ...
    print(f"  Target users   : {len(generation_plan)} (ranks #4-53)")
    print(f"  Multiplier     : {MULTIPLIER}x")
